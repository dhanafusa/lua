...
-- vimopt setting
set.number = true --行番号
set.signcolumn = 'yes' --行番号の横に余白
set.relativenumber = true --行番号の相対表示
set.expandtab = true --tabを空白に
set.tabstop = 4 -- インデント幅を4に
set.shiftwidth = 4 -- インデント幅を4に
set.autoindent = true -- 次の行にインデント引き継ぎ
set.smartindent = true -- 言語に合わせてインデント調整
set.wrap = true --折り返し
set.mouse = 'a' --マウス有効

-- map setting
map('i','jj','<Esc>',opts) -- jjでesc
map("n", "<Leader>e", ":Neotree toggle<Return>", opts) -- Leader + eでNeotree toggle
map('n', '<leader><leader>', ':Telescope find_files<Return>',opts)

-- neotree config setting
local config = {
